...
namespace ILRepacking
{
    internal class RepackImporter : IRepackImporter, IRepackCopier
    {
        ...
        public InterfaceImplementation Import(InterfaceImplementation reference, IGenericParameterProvider context)
        {
            ...
            var typeRef = reference.InterfaceType;
            typeRef = Import(typeRef, context);
            ...
            if (debugInformation.HasCustomDebugInformations)
            {
                foreach (var customDebugInformation in debugInformation.CustomDebugInformations)
                {
                    ...
                    if (customDebugInformation is AsyncMethodBodyDebugInformation)
                    {
                        ...
                        var newAsyncDebugInformation = new AsyncMethodBodyDebugInformation(asyncDebugInformation.CatchHandler.Offset != -1 ? GetInstruction(body, nb, asyncDebugInformation.CatchHandler.Offset).Offset : -1);
                        ...
                        newAsyncDebugInformation.MoveNextMethod = asyncDebugInformation.MoveNextMethod;
                    ...
                    else if (customDebugInformation is StateMachineScopeDebugInformation)
                    {
                        ...
                        var startInstr = GetInstruction(body, nb, stateMachineDebugInformation.Start.Offset);
                        var endInstr = GetInstruction(body, nb, stateMachineDebugInformation.End.Offset);

                        if (startInstr != null)
                        {
                            newCustomDebugInformation = new StateMachineScopeDebugInformation(
                                startInstr,
                                endInstr);
                        }
                        else
                        {
                            newCustomDebugInformation = new StateMachineScopeDebugInformation(
                                stateMachineDebugInformation.Start.Offset,
                                stateMachineDebugInformation.End.Offset);
                        }
                    }
