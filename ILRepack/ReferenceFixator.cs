            ...
            if (debugInformation.HasCustomDebugInformations)
            {
                foreach (var customDebugInformation in debugInformation.CustomDebugInformations)
                {
                    if (customDebugInformation is AsyncMethodBodyDebugInformation)
                    {
                        ...
                        if (asyncDebugInformation.ResumeMethods != null)
                        {
                            var fixedMethods = asyncDebugInformation.ResumeMethods.Select(rm => Fix(rm, true).Resolve()).ToArray();
                            asyncDebugInformation.ResumeMethods.Clear();

                            foreach (var method in fixedMethods)
                            {
                                asyncDebugInformation.ResumeMethods.Add(method);
                            }
                        }
                    }
